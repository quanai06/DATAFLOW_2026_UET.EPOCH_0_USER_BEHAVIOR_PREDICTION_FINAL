...
- Hybrid Ensemble: Kết hợp 10 mô hình (5-Fold Cross Validation x 2 kiến trúc LSTM và GRU).

- Split-Head Architecture: Tách biệt nhánh dự báo Ngày/Tháng (Sigmoid) và Công suất (Deep Dense) để tối ưu hóa cho từng loại mục tiêu.

- Attention Mechanism: Trực quan hóa các "điểm chạm" quan trọng trong chuỗi hành động (ví dụ: mã 783).
- Wide & Deep: Hợp nhất vector ngữ cảnh từ RNN với 17 đặc trưng thống kê (Velocity, Top 10 Hubs, cờ hiệu 783)
...
### Clone dự án
```bash
git clone https://github.com/quanai06/DATAFLOW_2026_UET.EPOCH_0_USER_BEHAVIOR_PREDICTION_FINAL
cd DATAFLOW_2026_UET.EPOCH_0_USER_BEHAVIOR_PREDICTION_FINAL
```
